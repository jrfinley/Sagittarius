...
{
    private Stack<Item_Temp> items;//handles all items a stack contains * .push = add to stack, .pop = remove top item from stack

    public Stack<Item_Temp> Items
    {
        get { return items; }
        set { items = value; }
    }
    ...
    public void AddItem(Item_Temp item)//adds more items of same type to stack, note: there is a stack cap in this system
    {
        items.Push(item);
    ...
    public void AddItems(Stack<Item_Temp> items)//allows stack allocation of items
    {
        this.items = new Stack<Item_Temp>(items);
